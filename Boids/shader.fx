...
float4 DoSpecular(Light lightObject, float3 vertexToEye, float3 lightDirectionToVertex, float3 Normal)
{
	...
	// ***************************************** 
	/*float3 light = normalize(-lightDirectionToVertex);
	float3 normal = normalize(Normal);
	float3 r = normalize(2 * dot(light, normal) * normal - light);
	float3 v = vertexToEye;

	float dotProduct = dot(r, v);
	float Shininess = 20;
	float4 SpecularColor = float4(1, 1, 1, 1);
	float SpecularIntensity = 2;
	float4 specular = SpecularIntensity * SpecularColor * max(pow(dotProduct, Shininess), 0) * 1;////length(input.Color);*/
...
LightingResult DoPointLight(Light light, float3 vertexToEye, float4 vertexPos, float3 N)
{
	...
	float distance = 10000;// length(LightDirectionToVertex);
